...
ansible-playbook -i ansible/localhost --become-method=sudo --extra-vars "deploy_user=mockup" -t "add_new_user, copy_pub_key, generate_ssh_keys, create_dir" ansible/usersetup.yml
rm -rf /tmp/mockup_db.sql
rsync -avz --stats --progress target/test-1.0-SNAPSHOT-jar-with-dependencies.jar mockup@localhost:/home/mockup/deploy/
